...
import com.eldritch.scifirpg.proto.Items.Item.Requirement;
import com.eldritch.scifirpg.proto.Items.Item.Type;
...
public class ItemTable extends MajorAssetTable<Item> {
	...
	private static class ItemRequirementEditorPanel extends AssetEditorPanel<Requirement, ItemRequirementTable> {
		private static final long serialVersionUID = 1L;

		private final JComboBox<Discipline> disciplineBox = new JComboBox<>(Discipline.values());
		private final JTextField valueField = new JTextField();

		public ItemRequirementEditorPanel(ItemRequirementTable owner, JFrame frame, Optional<Requirement> prev) {
			super(owner, frame, prev);
		...
		public Requirement createAsset() {
			...
			Requirement req = Requirement.newBuilder()
	...
	private static class DamageModEditorPanel extends AssetEditorPanel<DamageMod, DamageModTable> {
		...
		public DamageModEditorPanel(DamageModTable owner, JFrame frame, Optional<DamageMod> prev) {
			...
			damageBox.setModel(new DefaultComboBoxModel<DamageType>(values.toArray(new DamageType[0])));
