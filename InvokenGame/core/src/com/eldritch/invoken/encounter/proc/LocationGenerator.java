...
public class LocationGenerator {
	...
	public LocationGenerator(Biome biomeType) {
		...
		midWallTop = new StaticTiledMapTile(atlas.findRegion(biome
				+ MID_WALL_TOP));
		// midWallCenter = new StaticTiledMapTile(atlas.findRegion(biome +
		// MID_WALL_CENTER));
		midWallBottom = new StaticTiledMapTile(atlas.findRegion(biome
				+ MID_WALL_BOTTOM));
		...
		narrowWall = merge(rightTrim.getTextureRegion(),
				leftTrim.getTextureRegion());
	...
	public Location generate(com.eldritch.invoken.proto.Locations.Location proto) {
		...
		TiledMapTile roofTile = new StaticTiledMapTile(atlas.findRegion(biome
				+ ROOF));
		...
		for (LocationLayer layer : createSpawnLayers(base, collision,
				bsp.getRooms(), proto.getEncounterList(), map)) {
		...
		IcarianFurnitureGenerator furnitureGenerator = new IcarianFurnitureGenerator(
				atlas, ground);
	...
	private Map<NaturalVector2, Room> createRooms(List<Rectangle> bounds,
			CellType[][] typeMap) {
	...
	private LocationLayer createBaseLayer(CellType[][] typeMap, int width,
			int height, LocationMap map) {
	...
	private LocationLayer createEmptyLayer(LocationLayer base, LocationMap map,
			String name) {
		LocationLayer layer = new LocationLayer(base.getWidth(),
	...
	private LocationLayer createTrimLayer(LocationLayer base,
		...
		for (int x = 0; x < base.getWidth(); x++) {
			for (int y = 0; y < base.getHeight(); y++) {
				if (base.getCell(x, y) == null) {
					boolean leftGround = x - 1 >= 0
							&& base.getCell(x - 1, y) != null
							&& overlay.getCell(x - 1, y) == null;
					boolean rightGround = x + 1 < base.getWidth()
							&& base.getCell(x + 1, y) != null
							&& overlay.getCell(x + 1, y) == null;
					if (leftGround) {
						if (rightGround) {
	...
	private LocationLayer createOverlayLayer(LocationLayer base, LocationMap map) {
		...
		TiledMapTile belowTrim = new StaticTiledMapTile(atlas.findRegion(biome
				+ OVERLAY_BELOW_TRIM));
		for (int x = 0; x < base.getWidth(); x++) {
			for (int y = 0; y < base.getHeight(); y++) {
	...
	private LocationLayer createOverlayTrimLayer(LocationLayer base,
			LocationLayer roof, LocationLayer overlay, LocationMap map) {
		LocationLayer layer = new LocationLayer(base.getWidth(),
				base.getHeight(), PX, PX, map);
		layer.setVisible(true);
		layer.setOpacity(1.0f);
		...
		// bottom left
		TiledMapTile leftCorner = new StaticTiledMapTile(atlas.findRegion(biome
				+ LEFT_CORNER));
	...
	private CollisionLayer createCollisionLayer(LocationLayer base,
			LocationMap map) {
		CollisionLayer layer = new CollisionLayer(base.getWidth(),
				base.getHeight(), PX, PX, map, collider);
	...
	private List<LocationLayer> createSpawnLayers(LocationLayer base,
			LocationLayer collision, List<Rectangle> rooms,
			List<Encounter> encounters, LocationMap map) {
		List<Encounter> availableEncounters = new ArrayList<Encounter>(
				encounters);
		...
		for (Rectangle room : rooms) {
			...
			if (playerLayer == null) {
				playerLayer = new LocationLayer(base.getWidth(),
						base.getHeight(), PX, PX, map);
				...
				Encounter encounter = popEncounter(room, availableEncounters,
						total);
				if (encounter != null) {
					LocationLayer layer = createLayer(encounter, room, base,
							collision, map);
					layers.add(layer);
				}
			}
	...
	private Encounter popEncounter(Rectangle room, List<Encounter> encounters,
			double total) {
	...
	private final LoadingCache<String, Room> availableRooms = CacheBuilder
			.newBuilder().build(new CacheLoader<String, Room>() {
	...
	private LocationLayer createLayer(Encounter encounter, Rectangle room,
			LocationLayer base, LocationLayer collision, LocationMap map) {
		...
		for (ActorScenario scenario : encounter.getActorParams()
				.getActorScenarioList()) {
			for (int i = 0; i < scenario.getMax(); i++) {
				NaturalVector2 position = it.hasNext() ? it.next() : getPoint(
						room, base, layer);
	...
	public static List<NaturalVector2> getFreeSpaces(LocationLayer layer,
			Rectangle bounds) {
		...
		for (int x = 0; x < layer.getWidth(); x++) {
			for (int y = 0; y < layer.getHeight(); y++) {
				...
				if (cell != null && cell.getTile() == ground) {
					...
					if (y + 2 < layer.getHeight()
							&& layer.getCell(x, y + 2) == null) {
	...
	public NaturalVector2 getPoint(Rectangle rect, LocationLayer base,
		...
		while (!queue.isEmpty()) {
			...
			if (base.isGround(point.x, point.y)
					&& !layer.hasCell(point.x, point.y)) {
	...
	private static Cell addCell(LocationLayer layer, TiledMapTile tile, int x,
			int y) {
	...
	private static TiledMapTile merge(TextureRegion left, TextureRegion right) {
		FrameBuffer buffer = new FrameBuffer(Format.RGB888, Settings.PX,
				Settings.PX, false);
		...
		TextureRegion leftPart = new TextureRegion(left, 0, 0, size,
				Settings.PX);
		TextureRegion rightPart = new TextureRegion(right, size, 0, size,
				Settings.PX);
		...
		batch.begin();
		batch.draw(leftPart, 0, 0, leftPart.getRegionWidth(),
				leftPart.getRegionHeight());
		batch.draw(rightPart, size, 0, rightPart.getRegionWidth(),
				rightPart.getRegionHeight());
		batch.end();
		buffer.end();
	...
	private void saveLayer(LocationLayer layer) {
		BufferedImage image = new BufferedImage(layer.getWidth(),
