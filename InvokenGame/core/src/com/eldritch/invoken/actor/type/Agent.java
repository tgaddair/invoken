...
public abstract class Agent extends CollisionEntity implements
	...
	private final LinkedList<Announcement> announcements = Lists
			.newLinkedList();
	...
	public Agent(ActorParams params, boolean unique, float x, float y,
			float width, float height, Level level,
	...
	public boolean isAimingAt(Agent other) {
		...
		if (targetCast(weaponSentry.getPosition(),
				weaponSentry.getTargetingVector())) {
	...
	public RayTarget getTargeting(Vector2 source, Vector2 target) {
		...
		return new RayTarget(targetingHandler.getTargeting(),
				targetingHandler.getFraction());
	}
	...
	public boolean isCompletelyDead() {
		...
		return !isAlive()
				&& animations.get(Activity.Death).get(direction)
		...
		final Iterator<Action> reverse = new Iterator<Action>() {
			...
			public void remove() {
				throw new UnsupportedOperationException(
						"Cannot remove from reverse iterator!");
			}
		};
	...
	public TextureRegionDrawable getPortrait() {
		TextureRegion region = animations.get(Activity.Idle)
				.get(Direction.Right).getKeyFrame(0);
	...
	private void addChoiceFor(Agent listener, Response response,
			List<BanterAnnouncement> banter) {
	...
	private void addResponseFor(Agent listener, Choice choice,
			List<BanterAnnouncement> banter) {
		Response response = getDialogueHandler().getResponseFor(choice,
				listener);
		if (response != null) {
	...
	protected void onDeath() {
		...
		releaseItems();
		announcements.clear();
		InvokenGame.SOUND_MANAGER.playAtPoint(getDeathSound(), getPosition(),
				3f);
	...
	public void update(float delta, Level level) {
		...
		if (isInteracting() && dst2(getInteractor()) > INTERACT_RANGE
				&& !forcedDialogue) {
			endJointInteraction();
		}
	...
	protected void updateDirection(Locatable observed) {
		if (observed != null) {
			...
			setDirection(getDominantDirection(weaponSentry.direction.x,
					weaponSentry.direction.y));
		}
	}
	...
	public void render(float delta, OrthogonalTiledMapRenderer renderer) {
		...
		if (isCloaked()) {
			...
			if (getLocation().getPlayer() == this || dst2 > maxDst2) {
				...
				float a = MathUtils.lerp(getCloakAlpha(), 1f, 1f - dst2
						/ maxDst2);
				setAlpha(a);
			}
		} else {
	...
	public void render(OrthogonalTiledMapRenderer renderer) {
		...
		} else if (motionTracker.isStanding()
				&& lastActivity == Activity.Explore) {
			lastStateTime = 0;
		}
	...
	private void render(Activity activity, Direction direction,
			float stateTime, OrthogonalTiledMapRenderer renderer) {
		// based on the actor state, get the animation frame
		TextureRegion frame = animations.get(activity).get(direction)
	...
	protected void draw(Batch batch, TextureRegion frame, Direction direction) {
		...
		batch.draw(frame, position.x - width / 2, position.y - height / 2,
				width, height);
	}
	...
	public Rectangle getBoundingBox(Rectangle rect) {
		float w = getWidth() / 3f;
		rect.set(position.x - w / 2, position.y - getHeight() / 2, w,
				getHeight() / 4);
		return rect;
	}
	...
	private class TargetingHandler implements RayCastCallback {
		...
		private int isTargeting(Fixture fixture) {
			...
			if (fixture.getUserData() != null
					&& fixture.getUserData() instanceof Agent) {
	...
	public class WeaponSentry implements TemporaryEntity {
		...
		public boolean hasLineOfSight(Agent target) {
			if (!lineOfSightCache.containsKey(target)) {
				...
				lineOfSightCache.put(target, Agent.this.hasLineOfSight(target)
		...
		private void updatePosition() {
			Vector2 origin = getRenderPosition();
			position.set(origin.x + direction.x, origin.y + direction.y).sub(
					offset);
	...
	/**
	 * Rotates towards the focus point, but does not immediately set itself
	 * there for a smoother effect.
	 */
	public class RotatingWeaponSentry extends WeaponSentry {
		...
		protected void updateDirection(float delta) {
			update(delta,
					destination.set(getRenderPosition()).add(getVelocity()));
		}
