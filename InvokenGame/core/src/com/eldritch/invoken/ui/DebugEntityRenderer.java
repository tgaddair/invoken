...
public class DebugEntityRenderer {
	...
	public void renderDispositions(Agent target, Collection<Agent> agents,
		...
		for (Agent other : agents) {
			...
			float saturation = 1;
			float brightness = 1;
			java.awt.Color hsv = java.awt.Color.getHSBColor(hue, saturation,
					brightness);
			Color c = new Color(hsv.getRed() / 255f, hsv.getGreen() / 255f,
					hsv.getBlue() / 255f, 1f);
			sr.setColor(c);
			...
			batch.begin();
			debugFont.draw(batch, String.format("%.2f", relation), screen.x,
					screen.y);
			batch.end();
		}
		sr.end();
	...
	public void renderLineOfSight(Agent target, Collection<Agent> agents,
			OrthographicCamera camera) {
		sr.setProjectionMatrix(camera.combined);
		sr.begin(ShapeType.Line);
		for (Agent other : agents) {
	...
	public void renderEnemies(Agent target, Collection<Agent> agents,
			OrthographicCamera camera) {
		sr.setProjectionMatrix(camera.combined);
		sr.begin(ShapeType.Line);
		for (Agent other : agents) {
	...
	public void renderVisible(Agent target, Collection<Agent> agents,
			OrthographicCamera camera) {
		sr.setProjectionMatrix(camera.combined);
		sr.begin(ShapeType.Line);
	...
	public void drawBetween(Vector2 source, Vector2 target,
			OrthographicCamera camera) {
		sr.setProjectionMatrix(camera.combined);
		sr.begin(ShapeType.Line);
		Color c = Color.RED;
		...
		for (FixedPoint point : coverPoints) {
			...
			if (target != null) {
				sr.setColor(target.getLocation().hasLineOfSight(target,
						target.getPosition(), position) ? Color.GREEN
						: Color.RED);
			}
	...
	public void renderThreat(Agent target, OrthographicCamera camera) {
		...
		Npc npc = (Npc) target;
		renderCircle(npc.getPosition(), NpcThreatMonitor.ALERT_RADIUS,
				camera.combined, Color.RED);
		renderCircle(npc.getPosition(), NpcThreatMonitor.SUSPICION_RADIUS,
				camera.combined, Color.ORANGE);
	}
	...
	private void renderCircle(Vector2 position, float radius,
			Matrix4 projection, Color color) {
