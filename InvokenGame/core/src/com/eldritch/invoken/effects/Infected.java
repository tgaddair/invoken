...
public class Infected extends AnimatedEffect {
     ...
     *            damage per second
     * @param duration
     *            seconds of continuous infection
     */
    public Infected(Agent target, Set<Agent> immune, Damage damage, Vector2 contact,
    ...
    public void update(float delta) {
        ...
        target.damage(damage, contact, delta);
        ...
        for (Agent neighbor : target.getNeighbors()) {
            if (!immune.contains(neighbor) && !neighbor.isToggled(Infected.class)) {
                ...
                if (neighbor.inRange(target.getPosition(), radius)) {
                    ...
                    immune.add(neighbor);
                    neighbor.addEffect(new Infected(neighbor, immune, damage, target.getPosition(),
                            duration, radius));
                }
            }
        }
